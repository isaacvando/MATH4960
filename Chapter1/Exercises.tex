...
\begin{document}
  ...
  \exercise{1.2.iv} \hl{Question: What are the monomorphisms in \cat{Field}?}
  ...
  \exercise{1.2.vii} For a given subset of objects $A \subseteq P$, consider $M_A = \{ x \in \mathrm{obj}P | \forall a \in A, \exists f \in P(a,x) \}$, i.e. the set of all elements in $P$ targeted by a morphism from every element in $A$. If it exists \hl{(when could the supremum not exist?)}, the supremum of $A$ is the least element in $M_A$ w.r.t. $\le$. 
  ...
  \exercise{1.3.iii} \hl{Question: Give an example of a functor such that its image is not a subcategory of the codomain.}
  ...
  Now pick a pair of groups such that the cardinalities of their sets of conjugacy classes differ. \hl{(How do I conclude that they are not isomorphic?)}.
  ...
  \exercise{1.4.ii} Pick two groups $H, K$ and two functors $F, G : \text{B}H \rightrightarrows \text{B}K$. By exercise 1.3.i, $F$ and $G$ are simply group homomorphisms. Now pick a natural transformation $\alpha : F \Rightarrow G$. $\alpha$ is just a mapping between group homomorphisms 

  \exercise{1.4.iv} For natural transformations 
  ...
  Are there notions of morphisms between objects in different categories? (Not functors, but a morphism from an object $x \in C$ to an object $y \in D$).
