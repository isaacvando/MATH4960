...
\begin{document}
  ...
  *\exercise{1.2.v} In \cat{Ring}, consider the inclusion map $i : \Z \hookrightarrow \Q$. Now pick two morphisms, $f,g : R \rightrightarrows \Z$ such that $if = ig$ where $R$ is some ring. Now BWOC suppose that $f \ne g$. Then there exists an $r \in R$ such that $f(r) \ne g(r)$. Hence $i(f(r)) \ne i(g(r))$ so $if \ne ig$ which is a contradiction. Hence the above implies that $f = g$ so $i$ is monic. 
  ...
  \exercise{1.7.ii} Observe that the following diagram commutes as it is the diagram defining horizontal composition:
  \begin{center}
    \begin{tikzcd}
      H(Fc) & K(Fc)
    \end{tikzcd}
  \end{center}
  Hence by funtoriality of $L$,
  \begin{align*}
    L(\beta_{Fc'} \circ HFf) &= L(KFf \circ \beta_{Fc}) \\
    L(\beta_{Fc'})L(HFf) &= L(KFf)L(\beta_{Fc})
  \end{align*}
  so the diagram commutes and $L\beta F$ is natural.
  ...
  Now, for each $c \in C$, pick a functor $K_c : D \ra E$. Then we may define $F : C \times D \ra E$ for objects as $(c,d) \mapsto K_c(d)$. For a morphism $(f,g) : (c,d) \ra (c',d')$, define $F(f,g)$ as $(f,g) \mapsto K_c(g)$ -- not quite. This is where the natural transformation comes in to play
